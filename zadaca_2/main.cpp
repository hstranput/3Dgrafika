...
void draw_image(Objects objs, Lights lights)
{
    ...
    for (int j = 0; j < height; ++j)
    {
        for (int i = 0; i < width; ++i)
        {
            ...
            Vec3f color = cast_ray(ray, objs, lights, 1);
            img[i + j * width] = color;
        }
    }
...
int main() {
    ...
    Cuboid c1(Vec3f(-30, -5, -30), Vec3f(30, -4.5, 9), grey);
    Sphere s1(Vec3f(0, -3.5, -12), 1, blue);
    Sphere s2(Vec3f(0, -3.5, -19), 2, green);
    Cuboid c2(Vec3f(7, 0,  -15), Vec3f(10, -7,  -10), red);
    Cuboid c3(Vec3f(-1, 0,  -15), Vec3f(-10, -7,  -10), red);

    Objects objs = { &s1, &s2, &c1, &c2, &c3 };


    Light l1 = Light(Vec3f(-20, 20, 20), 2000);
    Light l2 = Light(Vec3f(20, 30, 20), 3000);
    Lights lights = { &l1, &l2 };
